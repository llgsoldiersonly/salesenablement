...
import type { FirmInput, SiteScrapeData, SourceResult } from "../types.ts";
...
async function fetchWithLimits(url: string): Promise<{ html: string; status: number; finalUrl: string; bytes: number; ms: number }> {
...
export function parseSiteHtml(input: ParseInput): SiteScrapeData {
