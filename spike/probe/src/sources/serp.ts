import type { FirmInput, SerpCompetitor, SerpData, SourceResult } from "../types.ts";
...
function findFirmRank(
  ...
  const firmHost = (() => {
    try {
      return new URL(firm.url.startsWith("http") ? firm.url : `https://${firm.url}`).hostname.replace(/^www\./, "");
    } catch {
      return null;
    }
  })();
